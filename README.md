...
本リポジトリは非公開なので、GitHubにログインした状態で操作してください。

### 1. コードを入手する

**方法A: ZIPでダウンロード（Gitを使わない、一番簡単な方法）**

1. ブラウザでこのリポジトリのページを開く（GitHubアカウントでログイン済みであること）
2. 緑色の「Code」ボタン → 「Download ZIP」をクリック
3. ダウンロードしたZIPを展開する

**方法B: git cloneを使う（Gitがインストール済みの場合）**

```bash
git clone https://github.com/c-haruto/fugaku-job-success-predictor.git
```

初回はブラウザでGitHubへのログインを求められることがあります。

### 2. Pythonをインストールする

そのPCにPython（3.10以上）が入っていない場合:

1. https://www.python.org/downloads/ を開き、「Download Python」ボタンからインストーラーをダウンロード
2. インストーラー実行時に「**Add python.exe to PATH**」に必ずチェックを入れてからインストール
3. 確認のため、ターミナル（Windowsの場合はPowerShell）で以下を実行し、バージョンが表示されればOK
...
```bash
cd "展開/クローンした場所のパス\fugaku-job-success-predictor"
```
...
学習済みモデルはリポジトリに同梱済みなので、これだけで動きます。F-DATAの
ダウンロードや再学習は不要です。
...
`Uvicorn running on http://127.0.0.1:8000` のような表示が出れば起動成功です。
...
サーバーを止めるには、その画面で `Ctrl + C` を押します。
`uvicorn`コマンドが見つからない場合は、手順4の`pip install -e .`が正常に
完了しているか確認してください。
